...
public class FixedDepositServiceImpl implements FixedDepositService {
    ...
    public ResponseEntity<?> createFD(FDRequestDTO request) {
        ...
        double interestRate = getInterestRate(request.getTenure());

        fd.setInterestRate(interestRate);

        fd.setTenureYears(request.getTenure());
        ...
        LocalDateTime maturityDate =
                startDate.plusYears(request.getTenure().getMonths());

        fd.setMaturityDate(maturityDate);

        BigDecimal interest =
                (request.getDepositAmount().multiply(BigDecimal.valueOf(interestRate)).multiply(BigDecimal.valueOf(request.getTenure().getMonths()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP))); // 100;
